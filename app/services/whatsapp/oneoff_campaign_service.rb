...
class Campaign < ApplicationRecord
  ...
  def trigger!
    ...
    # Use transaction with pessimistic locking to prevent concurrent execution
    Campaign.transaction do
      # Reload with lock to prevent race conditions
      campaign = Campaign.lock.find_by(id: id)
      return unless campaign
      return if campaign.completed?

      # Execute campaign - service will mark as completed after successful processing
      execute_campaign
    end
  rescue ActiveRecord::RecordNotFound
    # Campaign was deleted, skip
    nil
  end
